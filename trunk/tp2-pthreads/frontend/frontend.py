...
class HandlerSO(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_HTML(self, error=False):
        self.send_response(200)
        self.send_header('Content-type','text/html')
        self.send_header('Content-encoding', 'utf8')
        self.end_headers()
        if error:
           self.wfile.write(open(folder + '/static/error.html').read())
        else:
...
if __name__ == '__main__':
    try:
        ...
        print "Error: se debe indicar el puerto en la linea de comandos!"
        sys.exit(1)

    try:
        print "Escuchando http://localhost:%s" % port
        run(server_address=('',port))
    except Exception, e:
        print "Error: %s" % e
